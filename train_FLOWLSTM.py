...
root_dir = '/media/kongyu/Black6TB/Data/UCF-101/FlowLSTM/train/'
test_dir = '/media/kongyu/Black6TB/Data/UCF-101/FlowLSTM/val/'
path_for_model = '/media/kongyu/Black6TB/kev1n/LSTM_memory_6/model/'
...
if pretrain:
    for e in range(epoch):
        ...
        if (test_accuracy >= best_result):
            ...
            torch.save(model.state_dict(), os.path.join(path_for_model, 'flow_lstm.pt'))
...
state_dict = torch.load(os.path.join(path_for_model, 'flow_lstm.pt'))
...
for e in range(epoch):
    ...
    if (test_accuracy >= best_result):
        best_result = test_accuracy
        torch.save(memory, os.path.join(path_for_model, 'flow_memory.pt'))
    print('Epoch [%d], Loss: %.4f, Train accuracy: %.4f, Test accuracy: %.4f, Best: %.4f' % (
    e, running_loss / num_batch, train_accuracy, test_accuracy, best_result))
