...
class Memory(nn.Module):
    def __init__(self, memory_size, key_dim, num_class, top_k = 64, inverse_temp = 1, age_noise=4.0, margin = 0.1):
    ...
    def build(self):
        ...
        self.keys_var = nn.Parameter(self.keys)
    ...
    def predict(self, x):
        ...
        cosine_similarity, topk_indices_var = torch.topk(scores, int(self.top_k/4), dim=1)
        # cosine_similarity, topk_indices_var = torch.topk(scores, int(self.top_k), dim=1)
        # softmax of cosine similarities - embedding
        weight_score = F.softmax(self.softmax_temperature * cosine_similarity)
        topk_indices = topk_indices_var.data

        score = torch.zeros(batch_size, self.num_class)
        if x.is_cuda:
            score = score.cuda()
        for i in range(batch_size):
            topk_values = self.values.index_select(0, topk_indices[i])
            unique_index = unique(topk_values)
            #print(unique_index)
            if x.is_cuda:
                unique_index = unique_index.cuda()
            for j in range(len(unique_index)):
                place_hold = (topk_values==unique_index[j])
                #print(type(unique_index[j]))
                score[i,unique_index[j]] = torch.sum(place_hold.float()*weight_score[i])
        #print(weight_score[0])
    ...
    def forward(self, x, y, predict=False):
        ...
        cosine_similarity, _ = torch.topk(scores, self.top_k, dim=1)

        scores_data =scores.detach().data
        _, topk_indices_var = torch.topk(scores_data, self.top_k, dim=1)
        ...
        if not predict:
            ...
            pos_score, _ = torch.topk(torch.mul(cosine_similarity, correct_mask_var), 1, dim=1)
            neg_score, _ = torch.topk(torch.mul(cosine_similarity, 1-correct_mask_var), 1, dim=1)
    ...
    def update(self, query, y, y_hat, y_hat_indices):
        ...
        if incorrect:
            ...
            age_with_noise = self.age
            #print(age_with_noise.size())
            #print(incorrect_size)
            if incorrect_size > 0:
                ...
                #print(incorrect_values.size())
                #print(self.values[oldest_indices].size())
                self.values[oldest_indices] = incorrect_values.expand_as(self.values[oldest_indices].transpose(0,1)).transpose(0,1)
